...
#define MWM_DECOR_BORDER               (1L << 1)
    ...
    atom_NET_WM_WINDOW_TYPE,
            ...
            atom_MB_WM_WINDOW_TYPE_MESSAGE,
    ...
    atom_MB_WM_WINDOW_TYPE_MESSAGE =
            XInternAtom(ui->xdpy, "_MB_WM_WINDOW_TYPE_MESSAGE", False);
    ...
    if (wm_name && streq(wm_name, "Openbox"))
        have_openbox_wm = True;
    ...
    if (class_hints) {
        class_hints->res_name = "keyboard";
        class_hints->res_class = "Keyboard";
        XSetClassHint(ui->xdpy, ui->xwin, class_hints);
        XFree(class_hints);
    }
    ...
    if (!ui->want_embedding) {
        ...
        if (have_ewmh_wm) {
            ...
            Atom states[] = {atom_NET_WM_STATE_ABOVE, atom_NET_WM_STATE_SKIP_TASKBAR, atom_NET_WM_STATE_SKIP_PAGER};
            ...
            if (have_matchbox_wm) {
                DBG("is matchbox use MESSAGE \n");

                XChangeProperty(ui->xdpy, ui->xwin,
                        atom_NET_WM_WINDOW_TYPE, XA_ATOM, 32,
                        PropModeReplace,
                        (unsigned char *) /*&atom_NET_WM_WINDOW_TYPE_TOOLBAR*/ &atom_MB_WM_WINDOW_TYPE_MESSAGE, 1);
            } else if (have_openbox_wm) {
                DBG("is openbox use toolbar \n");
                ...
                DBG("unknown window manager use dock \n");
                XChangeProperty(ui->xdpy, ui->xwin,
                        atom_NET_WM_WINDOW_TYPE, XA_ATOM, 32,
                        PropModeReplace,
...
mb_kbd_ui_event_loop(MBKeyboardUI *ui) {
    ...
    while (True) {
        ...
        if (get_xevent_timed(ui->xdpy, &xev, &tvt)) {
            //		printf("XEvent type: %x ,,,, %x ,,,, %x \n", xev.type, xev.xconfigure.window, CreateNotify) ;
            switch (xev.type) {
                case ButtonPress:
                ...
                case ConfigureNotify:
                    //			printf("ConfigureNotify %x , %x \n", xev.xconfigure.window, ui->xwin) ;
